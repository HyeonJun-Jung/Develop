...
bool TextureClass::LoadTarga(char* filename, int& height, int& width)
{
    ...
    // 이제 targa 형식이 거꾸로 저장되었으므로 올바른 순서로 targa 이미지 데이터를
    // targa 대상 배열에 복사합니다.
    for (int j = 0; j < height; j++)
    {
        for (int i = 0; i < width; i++)
        {
            ...
            // 인덱스를 다음 targa 데이터로 증가시킵니다.
            k += 4;
            index += 4;
        }

        // targa 이미지 데이터를 역순으로 읽은 후 열의 시작 부분에서 이전 행으로 다시 설정합니다.
        k -= (width * 8);
    }
