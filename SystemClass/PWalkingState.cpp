...
PlayerState* PWalkingState::UpdateInput(InputClass* input, Player* player, float fDeltaTime)
{
	if (input->m_curkeyboardState[DIK_SPACE] & 0x80)
	{
		return new PJumpingState();
	}
	else if (input->m_curkeyboardState[DIK_LSHIFT] & 0x80)
	{
		return new PRunningState();
	}
	...
	{
		player->SetVelocity(XMFLOAT3(0, -9.8, 0));
		player->SetPhysxVelocity(PxVec3(0, 0, 0));
	}

	return nullptr;
}

void PWalkingState::Update(InputClass* input, Player* player, float fDeltaTime)
{
	if (m_Velocity < 100)
